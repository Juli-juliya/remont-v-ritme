...
function MaterialForm({ rooms, onSave, expense = false, initial }) {
  ...
  const [name,setName]=useState(initial?.name||''); const [category,setCategory]=useState(initial?.category||(expense?'Прочее':'Материалы')); const [price,setPrice]=useState(initial?.price?String(initial.price):''); const [detail,setDetail]=useState(initial?.detail||''); const [roomIds,setRoomIds]=useState(initial?.roomIds||[]);
  const bought = initial?.bought ?? expense; const urgent = initial?.urgent || false;
