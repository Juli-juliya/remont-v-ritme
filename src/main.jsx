...
function App() {
  ...
  const daysUntil=value=>Math.ceil((dateOnly(value)-today)/86400000);
  const expectedProgress=t=>{const start=dateOnly(t.start), end=dateOnly(t.end); if(!start||!end||today<=start)return 0; if(today>=end)return 100; return Math.round(((today-start)/(end-start))*100)};
  const taskAttentionReason=t=>{if(t.done)return null; if(t.risk)return 'Отмечено как риск'; const left=daysUntil(t.end); if(left<0)return `Просрочено на ${Math.abs(left)} ${plural(Math.abs(left),'день','дня','дней')}`; if(left<=7)return `Дедлайн через ${left} ${plural(left,'день','дня','дней')}`; const expected=expectedProgress(t), actual=t.progress||0; if(expected>=30&&actual+20<expected)return `Отставание: ${actual}% из ожидаемых ${expected}%`; return null};
  const materialAttentionReason=m=>!m.bought&&m.urgent?'Купить в первую очередь':null;
  const attentionTasks=object.tasks.map(t=>({...t,attentionReason:taskAttentionReason(t)})).filter(t=>t.attentionReason);
  const attentionMaterials=object.materials.map(m=>({...m,attentionReason:materialAttentionReason(m)})).filter(m=>m.attentionReason);
  const attentionCount=attentionTasks.length+attentionMaterials.length;
  ...
  const updateMaterial=(id,patch)=>{updateObject(o=>({...o,materials:o.materials.map(m=>m.id===id?{...m,...patch}:m)}));setSheet(null)};
  const deleteMaterial=id=>{updateObject(o=>({...o,materials:o.materials.filter(m=>m.id!==id)}));setSheet(null)};
  ...
  const Home=()=> <>
    ...
    <div className="section-title"><div><p className="eyebrow">СЕГОДНЯ В ФОКУСЕ</p><h3>Что требует внимания</h3></div><span>{attentionCount}</span></div>
    <div className="focus-list">{attentionMaterials.slice(0,3).map(m=><button className="focus-card coral" key={`mat-${m.id}`} onClick={()=>setTab('materials')}><div className="focus-icon"><PackageCheck size={21}/></div><div><b>Купить: {m.name}</b><span>{m.attentionReason} · {roomNames(m.roomIds)}</span></div><ChevronRight size={19}/></button>)}{attentionTasks.slice(0,3).map(t=><button className="focus-card amber" key={`task-${t.id}`} onClick={()=>{setTab('works');setFilter('Риски')}}><div className="focus-icon"><Clock3 size={21}/></div><div><b>{t.title}</b><span>{t.attentionReason}</span></div><ChevronRight size={19}/></button>)}{attentionCount>6&&<div className="all-good">+ ещё {attentionCount-6} {plural(attentionCount-6,'пункт','пункта','пунктов')} требуют внимания</div>}{!attentionCount&&<div className="all-good"><Check size={18}/> Всё идёт спокойно</div>}</div>
  ...
  const Works=()=>{const visible=object.tasks.filter(t=>filter==='Все'||(filter==='В работе'?!t.done:!!taskAttentionReason(t)));return <><header className="page-head"><p className="eyebrow">{object.name}</p><h1>Работы</h1><p>{object.tasks.filter(t=>t.done).length} из {object.tasks.length} завершено</p></header><div className="segmented">{['Все','В работе','Риски'].map(f=><button className={filter===f?'active':''} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="timeline">{visible.map(t=><article className={`task-card ${t.done?'done':''}`} key={t.id}><button className="check" aria-label={t.done?`Вернуть «${t.title}» в работу`:`Завершить «${t.title}»`} onClick={()=>toggleTask(t.id)}>{t.done&&<Check size={16}/>}</button><div className="task-body"><div className="task-meta"><span>{t.group}</span>{taskAttentionReason(t)&&<em><CircleAlert size={12}/> {taskAttentionReason(t)}</em>}</div><h3>{t.title}</h3><p>{roomNames(t.roomIds)}</p><div className="task-bottom"><span><CalendarDays size={14}/>{shortDate(t.start)} — {shortDate(t.end)}</span><b>{money.format(t.cost)} ₽</b></div>{!t.done&&t.progress>0&&<div className="task-progress"><i style={{width:`${t.progress}%`}}/><span>{t.progress}%</span></div>}</div></article>)}{!visible.length&&<div className="empty"><Check/><b>Здесь пока пусто</b><span>Добавьте первую работу</span></div>}</div><button className="primary-button sticky-action" onClick={()=>setSheet('add-task')}><Plus size={18}/> Добавить работу</button></>};

  const Materials=()=> <><header className="page-head"><p className="eyebrow">{object.name}</p><h1>Покупки</h1><p>{object.materials.filter(m=>m.bought).length} из {object.materials.length} куплено</p></header><section className="material-summary"><PackageCheck size={30}/><div><span>Осталось купить</span><b>{money.format(object.materials.filter(m=>!m.bought).reduce((s,m)=>s+m.price,0))} ₽</b></div></section><div className="material-list">{object.materials.map(m=><article key={m.id} className={`material-row ${m.bought?'done':''}`}><button className="material-toggle" aria-label={m.bought?`Вернуть «${m.name}» в покупки`:`Отметить «${m.name}» купленным`} onClick={()=>toggleMaterial(m.id)}><span className="material-check">{m.category==='Мебель'?<Sofa size={18}/>:m.bought?<Check size={16}/>:<PackageCheck size={18}/>}</span><span className="material-copy"><span className="category-tag">{m.category}</span><b>{m.name}</b><small>{roomNames(m.roomIds)}{m.detail?` · ${m.detail}`:''}</small>{m.urgent&&!m.bought&&<em>Купить в первую очередь</em>}</span><strong>{money.format(m.price)} ₽</strong></button><div className="material-actions"><button onClick={()=>setSheet({type:'edit-material',id:m.id})}>Править</button><button className="danger-link" onClick={()=>deleteMaterial(m.id)}>Удалить</button></div></article>)}</div><button className="primary-button" onClick={()=>setSheet('add-material')}><Plus size={19}/> Добавить покупку</button></>;

  const selectedRoom=sheet?.type==='room'?object.rooms.find(r=>r.id===sheet.id):null;
  const selectedMaterial=sheet?.type==='edit-material'?object.materials.find(m=>m.id===sheet.id):null;
  return <div className="app-shell"><main>{tab==='home'?<Home/>:tab==='works'?<Works/>:<Materials/>}</main><nav className="bottom-nav"><button className={tab==='home'?'active':''} onClick={()=>setTab('home')}><House/><span>Главная</span></button><button className={tab==='works'?'active':''} onClick={()=>setTab('works')}><ListChecks/><span>Работы</span></button><button className="add-button" aria-label="Добавить" onClick={()=>setSheet('quick')}><Plus/></button><button className={tab==='materials'?'active':''} onClick={()=>setTab('materials')}><Layers3/><span>Покупки</span></button><button onClick={()=>setSheet('budget')}><ReceiptText/><span>Смета</span></button></nav>
    <Sheet open={!!sheet} onClose={()=>setSheet(null)} wide={['add-task','add-material','expense'].includes(sheet)||sheet?.type==='edit-material'}>
      ...
      {sheet==='add-task'&&<TaskForm rooms={object.rooms} onSave={addTask}/>} {sheet==='add-material'&&<MaterialForm rooms={object.rooms} onSave={addMaterial}/>} {sheet==='expense'&&<MaterialForm rooms={object.rooms} expense onSave={addMaterial}/>} {selectedMaterial&&<><MaterialForm rooms={object.rooms} initial={selectedMaterial} onSave={v=>updateMaterial(selectedMaterial.id,v)}/><button className="danger-button" onClick={()=>deleteMaterial(selectedMaterial.id)}><Trash2 size={17}/> Удалить покупку</button></>}
